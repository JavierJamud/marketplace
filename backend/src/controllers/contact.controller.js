...
export async function sendContactMessage(req, res) {
  ...
  if (!admin?.email) throw new AppError("No se pudo enviar el mensaje. Probá de nuevo más tarde.", 503);

  const result = await sendContactMessageEmail({ to: admin.email, name, email, message });
  if (!result.ok) throw new AppError("No se pudo enviar tu mensaje. Probá de nuevo en un momento.", 502, { detail: result.error });
